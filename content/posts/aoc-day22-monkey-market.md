...
It's a simple enough process and careful reading lets it be written directly.
...
```swift
  func evolutionFunction(n: Int) -> (Int) -> Int {
    { secretNumber in
...
```swift
  func part1() async throws -> Int {
    rows.map(evolutionFunction(n: 2000)).reduce(0, +)
  }
```
