# Regenerate posts from this file:
#   C-c C-e H H  export the current subtree only (org-hugo-export-wim-to-md) - avoid this unless just testing previews
#   C-c C-e H A  regenerate the whole site (org-hugo-export-wim-to-md :all-subtrees)

# Add new Post with capture templates
#   C-c c b t    Adds a new post drawer ready to be filled in.

#+HUGO_BASE_DIR: ./
#+options: author:nil
#+MACRO: marginnote @@html:{{<marginnote>}}$1{{</marginnote>}}@@
#+MACRO: sidenote @@html:{{<sidenote>}}$1{{</sidenote>}}@@
#+MACRO: newthought @@html:{{<newthought>}}$1{{</newthought>}}@@
#+MACRO: epigraph @@html:{{<epigraph author="$1" cite="$2" detail="$3">}}$4{{</epigraph>}}@@

# These are mostly used in the Talks section
#+MACRO: speakerdeck @@html:{{<marginnote>}}[Slides on Speaker Deck](https://speakerdeck.com/abizern/$1){{</marginnote>}}@@
...
# These are used in Advent of Code posts. $1 is the year, $2 is the day (aocday) or the path within the repo after /main/ (aocsrc)
#+MACRO: aocday @@html:{{<sidenote>}}https://adventofcode.com/$1/day/$2{{</sidenote>}}@@
#+MACRO: aocsrc @@html:{{<sidenote>}}https://github.com/Abizern/aoc-swift-$1/blob/main/$2{{</sidenote>}}@@
...
After spending far too long yesterday unsuccessfully trying to get my head around nested recursions, today{{{aocday(2024,22)}}} was something a little easier.
...
*** Final Thoughts
The full code is on Github{{{aocsrc(2024,Sources/Day22.swift)}}}.
...
We have another maze{{{aocday(2024,20)}}} problem, with the ability of cutting through walls to give shorter paths.
...
*** Final Thoughts
For context, the full code is on Github{{{aocsrc(2024,Sources/Day20.swift)}}}.
...
My worries for today's{{{aocday(2024,19)}}} puzzle being very difficult were unfounded. Which makes me more concerned for the weekend.
...
As usual, the full source is available on Github{{{aocsrc(2024,Sources/Day19.swift)}}}.
...
Unlike Reindeer Maze{{{aocday(2024,16)}}}, this is a pure path finding problem. I don't need to implement the search myself{{{sidenote(Not wanting to do this myself is one of the reasons I haven't even started solving Day 16 yet)}}}, I can just use the built in methods from GameplayKit.
...
The full code{{{aocsrc(2024,Sources/Day18.swift)}}} has more context around the helper functions, but this is quite simple:
...
I actually got this{{{aocday(2024,15)}}} done on the day, but haven't written it up because I'm not too happy with the way I got to my solution{{{aocsrc(2024,Sources/Day15.swift)}}}.
...
I think this{{{aocday(2024,14)}}} was a short one because part2 wasn't the easiest to come up with a definitive answer.
...
It probably isn't worth putting the code itself here, but it's available{{{aocsrc(2024,Sources/Day14.swift)}}} on Github.
...
This one is just about maths. We have a machine with buttons to move a claw{{{aocday(2024,13)}}} and want to know a) can it be positioned in a particular place, and b) if it can be positioned, how much will it cost.
...
As usual, the full code{{{aocsrc(2024,Sources/Day13.swift)}}} is on Github.
...
Today's challenge{{{aocday(2024,12)}}} felt very strange to me. I read the question. I knew what I had to do for Part 1, but I didn't feel very motivated to actually finish my implementation. I pushed through and eventually got it done, then spent too long thinking about how to do Part 2 before I realised that it was more or less the same approach as for part 1, just with different parameters.
...
As usual, the full code for this is on Github{{{aocsrc(2024,Sources/Day12.swift)}}}. It felt like a slog, I don't mind telling you.
...
Today{{{aocday(2024,11)}}} we had a list of stones that changed over time and we had to count how many there would be at the end of the count. I thought this sort of problem would come up, I've seen it before{{{sidenote(The Lanternfish from 2021 https://adventofcode.com/2021/day/6)}}}, and it is one of my favourite ones because it was the first time I saw the process for solving these puzzles.
...
The full source, which is not much longer, is available on Github{{{aocsrc(2024,Sources/Day11.swift)}}}.
...
I was expecting a Graph{{{aocday(2024,9)}}} problem to show up around now, and it as a good time to create a utility =Grid= class to make working with these 2D graphs a little easier.
...
*** Tidying Up
The code is so similar that I rewrote it to a single function. The full solution is on Github{{{aocsrc(2024,Sources/Day10.swift)}}}.
...
I had a bit of difficulty today{{{aocday(2024,9)}}} for two reasons. Firstly, Swift doesn't seem to be that good with deep recursions. I wanted to use a recursive solution, but my stack size grow too large. Secondly, I didn't read the requirements for part 2 properly, and it took me a while to figure out how to bubble files up into the empty slots.

I eventually got it done with an imperative loop{{{aocsrc(2024,Sources/Day09.swift)}}}
...
We are given a grid of antennas{{{aocday(2024,8)}}} and we're supposed to find which ones line up and find points that extend from them, and count the unique positions where they occur.
...
I'm not going to show the code here, if you'd like to see it, the solution is online{{{aocsrc(2024,Sources/Day08.swift)}}}. I'll concentrate on the reasoning.
...
Today{{{aocday(2024,7)}}} was about trying to validate lists of numbers according to simple rules.
...
I don't normally include a lot of tests in my solutions - I test the parsing and the example answers, and the correct result is another test. That's usually enough. My solution{{{aocsrc(2024,Sources/Day04.swift)}}} I had to write tests{{{aocsrc(2024,Tests/Day07Tests.swift#L22-L35)}}} for my validation code, which pointed out that I was taking numbers from the wrong end.
...
Another{{{aocday(2024,6)}}} grid traversal and the longest solution{{{aocsrc(2024,Sources/Day06.swift)}}} I've had to write so far.
...
This was another day{{{aocday(2024,5)}}} where part 2 wasn't as much of a jump in difficulty, but needed careful reading; only add the middle values for lists that need sorting. You can see my full solution on Github{{{aocsrc(2024,Sources/Day05.swift)}}}.
...
Today{{{aocday(2024,4)}}} wasn't so much about graph theory once you read the questions, but I took a similar approach to solving the problem. Parsing was trivial; just read a nested array of Characters.

My solution can be found on Github{{{aocsrc(2024,Sources/Day04.swift)}}}
...
Normally it takes a few more days before I feel my Advent of Code inadequacy, but it struck on day 3{{{aocday(2024,3)}}} instead. Parsing the input into a data structure that I could work with was the hardest part of today's challenge.
...
Leaving aside the parsing for now, I'll discuss my solutions{{{aocsrc(2024,Sources/Day03.swift)}}}
...
Today's{{{aocday(2024,2)}}} challenge was only slightly more complicated than yesterday's, and one where brute(ish) force was enough. My solution is on Github{{{aocsrc(2024,Sources/Day02.swift)}}}.
...
As expected the first day's{{{aocday(2024,1)}}} problem is fairly easy and a way to make sure that one's environment is set up correctly and works.
...
The full solution is available on Github{{{aocsrc(2024,Sources/Day01.swift)}}}.
...
Move beyond "LGTM". Pull Requests{{{speakerdeck(pull-requests-are-a-language)}}}{{{skillsmatter(https://skillsmatter.com/skillscasts/11710-lightning-talk-pull-requests-are-language#video)}}} can be a way for teams to communicate more than just changes in a codebase.

** The Road to Damascus, or FRP and Me
/NSSpain, Logroño, 14th September 2017/

A talk{{{speakerdeck(the-road-to-damascus-or-frp-and-me)}}}{{{marginnote([Video on NSSpain's Vimeo](https://vimeo.com/235050604))}}} given at NSSpain about my experiences of looking into using Functional Reactive Programming with RxSwift.

** Be Your Own Backend Developer
/Code Mobile, Chester, 18th April 2017/

An overview{{{speakerdeck(be-your-own-backend-developer)}}} of Server Side Swift. This was recorded, but unfortunately the video isn't available.

** Safer Programming with Types
/Swift London, London, 25th October, 2016/

A short{{{speakerdeck(safer-programming-with-types)}}}{{{youtube(https://www.youtube.com/watch?v=uR6ox-9VPAA&t=1s)}}} talk about type safety presented at the Swift London meetup.

** Core Data: It's not Dead Yet
/FrenchKit, Paris, 23rd September 2016/

I spoke{{{speakerdeck(core-data-its-not-dead-yet)}}}{{{youtube(https://youtu.be/iOdg7WOEWWE)}}} at the first FrenchKit (now known as Swift Connection) conference about how Core Data is still relevant in the age of Swift.

** Being Imperfect in a Perfect World
/NSBarcelona, Barcelona, 21st September 2016/

I spoke{{{speakerdeck(being-imperfect-in-a-perfect-world)}}} at the NSBarcelona meetup about the choices we make about the patterns we use in development.
...
An updated{{{speakerdeck(dependency-management-with-carthage)}}}{{{marginnote([Video on Vimeo](https://vimeo.com/album/3943556/video/165920026))}}} and expanded version of my previous talk about using Carthage for dependency management.

** Carthage
/NSLondon, London, 30th July, 2015/

A talk{{{speakerdeck(carthage)}}} at NSLondon about using Carthage for dependency management.

** What Haskell Teaches me about Writing Swift
/AltConf, San Francisco, June 2015/

I had the{{{speakerdeck(what-haskell-teaches-me-about-writing-swift)}}} opportunity to present a longer version of the lightning talk I gave at Swift Summit. These are my personal thoughts about writing Swift after knowing some Haskell.
...
A short talk{{{speakerdeck(what-haskell-taught-me-about-writing-swift)}}} about how knowing Haskell helped me to better understand and learn Swift.

** Fizz Buzz in Swift
/NSLondon, London, 14th August 2014/

A talk{{{speakerdeck(fizzbuzz-in-swift-a-talk-with-3-codas)}}} I gave about different approaches to solving the Fizz Buzz problem in in Swift, along with some thoughts about the new language.
...
I spoke at the first edition of iOSCon{{{speakerdeck(adding-3rd-party-code-to-xcode-projects)}}}{{{skillsmatter(https://skillsmatter.com/skillscasts/5058-third-party-code)}}} covering the usage of Git and CocoaPods for adding dependencies to an Xcode project
